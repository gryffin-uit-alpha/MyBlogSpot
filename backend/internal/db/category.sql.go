...
const listCategoriesWithCount = `-- name: ListCategoriesWithCount :many
SELECT
  c.id,
  c.name,
  c.slug,
...
type ListCategoriesWithCountRow struct {
...
func (q *Queries) ListCategoriesWithCount(ctx context.Context) ([]ListCategoriesWithCountRow, error) {
	rows, err := q.db.Query(ctx, listCategoriesWithCount)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListCategoriesWithCountRow{}
	for rows.Next() {
		var i ListCategoriesWithCountRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.Slug,
